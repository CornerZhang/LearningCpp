...
namespace {
     ...
     */
    void log(const char * format, ...) NC_FORMAT_PRINTF(1, 2);
...
{
    if(_running) {
        ::log("Console already started. 'stop' it before calling 'listen' again");
        return false;
    }
...
void NetConsole::log(const char* buf)
{
...
void NetConsole::createCommandAllocator()
{
    addCommand({"allocator", "Display allocator diagnostics for all allocators. Args: [-h | help | ]",
        NC_CALLBACK_2(NetConsole::commandAllocator, this)});
}

void NetConsole::createCommandConfig()
{
    addCommand({"config", "Print the Configuration object. Args: [-h | help | ]",
        NC_CALLBACK_2(NetConsole::commandConfig, this)});
}

void NetConsole::createCommandDebugMsg()
{
    addCommand({"debugmsg", "Whether or not to forward the debug messages on the console. Args: [-h | help | on | off | ]",
        NC_CALLBACK_2(NetConsole::commandDebugMsg, this)});
    addSubCommand("debugmsg", {"on", "enable debug logging", NC_CALLBACK_2(NetConsole::commandDebugMsgSubCommandOnOff, this)});
    addSubCommand("debugmsg", {"off", "disable debug logging", NC_CALLBACK_2(NetConsole::commandDebugMsgSubCommandOnOff, this)});
}

void NetConsole::createCommandExit()
{
    addCommand({"exit", "Close connection to the console. Args: [-h | help | ]", NC_CALLBACK_2(NetConsole::commandExit, this)});
}

void NetConsole::createCommandFps()
{
    addCommand({"fps", "Turn on / off the FPS. Args: [-h | help | on | off | ]", NC_CALLBACK_2(NetConsole::commandFps, this)});
    addSubCommand("fps", {"on", "Display the FPS on the bottom-left corner.", NC_CALLBACK_2(NetConsole::commandFpsSubCommandOnOff, this)});
    addSubCommand("fps", {"off", "Hide the FPS on the bottom-left corner.", NC_CALLBACK_2(NetConsole::commandFpsSubCommandOnOff, this)});
}

void NetConsole::createCommandHelp()
{
    addCommand({"help", "Print this message. Args: [ ]", NC_CALLBACK_2(NetConsole::commandHelp, this)});
}


// commands
void NetConsole::commandAllocator(int fd, const std::string& args)
{
//#if CC_ENABLE_ALLOCATOR_DIAGNOSTICS
//    auto info = allocator::AllocatorDiagnostics::instance()->diagnostics();
//    ::consolePrintf(fd, info.c_str());
//#else
//    ::consolePrintf(fd, "allocator diagnostics not available. CC_ENABLE_ALLOCATOR_DIAGNOSTICS must be set to 1 in ccConfig.h\n");
//#endif
}

void NetConsole::commandConfig(int fd, const std::string& args)
{
//    Scheduler *sched = Director::getInstance()->getScheduler();
//    sched->performFunctionInCocosThread( [=](){
//        ::consolePrintf(fd, "%s", Configuration::getInstance()->getInfo().c_str());
//        Console::Utility::sendPrompt(fd);
//    });
}

void NetConsole::commandDebugMsg(int fd, const std::string& args)
{
    ::consolePrintf(fd, "Debug message is: %s\n", _sendDebugStrings ? "on" : "off");
}

void NetConsole::commandDebugMsgSubCommandOnOff(int fd, const std::string& args)
{
    _sendDebugStrings = (args.compare("on") == 0);
}

void NetConsole::commandExit(int fd, const std::string& args)
{
    FD_CLR(fd, &_read_set);
    _fds.erase(std::remove(_fds.begin(), _fds.end(), fd), _fds.end());
#if (NC_TARGET_PLATFORM == NC_PLATFORM_WIN32) || (NC_TARGET_PLATFORM == NC_PLATFORM_WINRT)
...
void NetConsole::commandFps(int fd, const std::string& args)
{
    ::log("FPS is: on");    // to debug windows
    ::consolePrintf(fd, "FPS is: %s\n", "on");    // to terminal
}

void NetConsole::commandFpsSubCommandOnOff(int fd, const std::string& args)
{
...
void NetConsole::commandHelp(int fd, const std::string& args)
{
